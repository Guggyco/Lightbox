...
open class HeaderView: UIView {
    ...
    open fileprivate(set) lazy var sendButton: UIButton = { [unowned self] in
        ...
        if let size = LightboxConfig.SendButton.size {
            button.frame.size = size
        } else {
            button.sizeToFit()
        }
        ...
        if let image = LightboxConfig.SendButton.image {
            button.setBackgroundImage(image, for: UIControlState())
        }
    ...
    public init() {
        ...
        [closeButton, deleteButton, sendButton].forEach { addSubview($0) }
    }
    ...
    @objc func sendButtonDidPress(_ button: UIButton) {
        delegate?.headerView(self, didPressSendButton: button)
    }
